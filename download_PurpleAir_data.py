...
def get_purpleair_sensor_ids(latitude, longitude, radius_meters=10000, indoor = True, api_key='8FF7CBA4-A704-11ED-B6F4-42010A800007'):
    ...
    params = {
        ...
        "location_type": int(indoor),  # 0 for outdoor sensors
        "max_distance": radius_meters,  # Search radius in meters
        "lat": latitude,
        "lon": longitude
    }
    df = pd.DataFrame(columns=['sensor_index', 'Name', 'latitude', 'longitude'])
    response = requests.get(url, headers=headers, params=params)
    
    if response.status_code == 200:
        sensors = response.json()["data"]
        return pd.DataFrame(sensors, columns=['sensor_index', 'name', 'latitude', 'longitude'])
    else:
